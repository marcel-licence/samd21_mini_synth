...
#ifndef SIMPLE_DELAY_BUFFER_SIZE
#define SIMPLE_DELAY_BUFFER_SIZE   SAMPLE_RATE /* results in one second long delay */
#endif

/*
 * module variables
 */
int16_t delayLine[SIMPLE_DELAY_BUFFER_SIZE];

int16_t delayToMix = 0;
int16_t delayFeedback = 0;
uint32_t delayLen = SIMPLE_DELAY_BUFFER_SIZE - 2;
uint32_t delayIn = 0;
uint32_t delayOut = 0;
...
{
    for (int i = 0; i < SIMPLE_DELAY_BUFFER_SIZE; i++)
    {
        delayLine[i] = 0;
...
void SimpleDelay_Process(int32_t *signal, uint32_t len)
{
    for (uint32_t i = 0; i < len; i++)
    {
        ...
        delayOut = delayIn + (1 + SIMPLE_DELAY_BUFFER_SIZE - delayLen);

        if (delayOut >= SIMPLE_DELAY_BUFFER_SIZE)
        {
            delayOut -= SIMPLE_DELAY_BUFFER_SIZE;
        }
        ...
        if (delayIn >= SIMPLE_DELAY_BUFFER_SIZE)
        {
            delayIn = 0;
        }
    }
...
void SimpleDelay_SetFeedback(uint8_t unused __attribute__((__unused__)), float value)
{
    delayFeedback = FLOAT_TO_I16(value);
    Serial.printf("delay feedback: %0.3f\n", value);
}

void SimpleDelay_SetLevel(uint8_t unused __attribute__((__unused__)), float value)
{
    delayToMix = FLOAT_TO_I16(value) / 0x100;
    Serial.printf("delay level: %0.3f\n", value);
}

void SimpleDelay_SetLength(uint8_t unused __attribute__((__unused__)), float value)
{
    delayLen = (uint32_t)(((float)SIMPLE_DELAY_BUFFER_SIZE - 1.0f) * value);
